...
def train_fn(X_train_shards, Y_train_shards, X_test, Y_test, return_dict,
    ...
    while return_dict['eval_success'] < gv.max_acc and t < args.T:
        ...
        # else:
        curr_agents = np.random.choice(agent_indices, num_agents_per_time, p=np.exp(guidance['gradient'][0]/guidance['gradient'][1])/np.sum(np.exp(guidance['gradient'][0]/guidance['gradient'][1]),axis=0), replace=False)
        ...
        while k < num_agents_per_time:
            ...
            for l in range(true_simul):
                ...
                guidance['gradient'][0][i] += np.sqrt( np.array([(x**2).mean() for x in return_dict[str(i)]]).mean() ); guidance['gradient'][1][i] += 1
                # p.start()
        ...
        if args.mal:
            ...
            eval_func(X_test, Y_test, t + 1, return_dict, mal_data_X, mal_data_Y, global_weights = global_weights)
        else:
            # p_eval = Process(target=eval_func, args=(
            #     X_test, Y_test, t + 1, return_dict), kwargs={'global_weights': global_weights})
            eval_func(X_test, Y_test, t + 1, return_dict, global_weights = global_weights)
        # p_eval.start()
        # p_eval.join()
    ...
    if args.train:
        ...
        for t in range(args.T):
            ...
            if args.mal:
                ...
                eval_func(X_test, Y_test_uncat, t, return_dict, mal_data_X, mal_data_Y)
            else:
            	# p_eval = Process(target=eval_func, args=(
